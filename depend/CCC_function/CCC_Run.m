...
% check CCC settings
if sum([Cfg.FlagStr Cfg.FlagSta Cfg.FlagVar Cfg.FlagSD Cfg.FlagtGV Cfg.FlagtMV])==0
    diary('off')
...
switch handles.DataType
    ...
    case {3,4}
        ...
        temp = dir(fullfile(DataPath{1},'*.mat'));%sublist index file
...
for isub = 1:SubNo
    ...
    fprintf('\n')
    % calculate DFC-str
    % FC(ifc).X(i,:) indicates the dynamic functional connectivity (ifc) of
    % subject (i)
    if Cfg.FlagStr
